...
/// Two blobs merging, drawn entirely from a distance field.
///
/// Used where the app needs something genuinely liquid — pieces combining into an
/// outfit, the closet absorbing a new garment.
struct MergingBlobs: View {
    /// 0 = apart, 1 = fully merged.
    var progress: Double
    ...
    var body: some View {
        TimelineView(.animation(paused: scenePhase != .active || reduceMotion)) { timeline in
            let time = timeline.date.timeIntervalSinceReferenceDate
            let gap = (1 - progress) * size * 0.34

            Rectangle()
                .colorEffect(
                    ShaderLibrary.metaballs(
                        .float2(CGPoint(x: size / 2 - gap, y: size / 2)),
                        .float(size * 0.19),
                        .float2(CGPoint(x: size / 2 + gap, y: size / 2)),
                        .float(size * 0.15),
                        // More smoothing as they close, so they reach for each
                        // other before touching — the bit that reads as liquid.
                        .float(size * 0.10 * (0.35 + progress)),
                        .float(time)
                    )
                )
        }
